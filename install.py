...
def prepare(rType="MAIN"):
    ...
    subprocess.check_call("sudo apt-get remove --auto-remove libcurl4 -y > /dev/null", shell=True)
    for rPackage in rPackages:
        printc("Installing %s" % rPackage)
        subprocess.check_call("sudo apt-get install %s -y > /dev/null" % rPackage, shell=True)
    printc("Installing libpng")
    subprocess.check_call("sudo wget -q -O /tmp/libpng12.deb http://mirrors.kernel.org/ubuntu/pool/main/libp/libpng/libpng12-0_1.2.54-1ubuntu1_amd64.deb", shell=True)
    subprocess.check_call("sudo dpkg -i /tmp/libpng12.deb > /dev/null", shell=True)
    subprocess.check_call("sudo apt-get install -y > /dev/null", shell=True) # Clean up abov, shell=Truee
    try: subprocess.check_call("sudo rm /tmp/libpng12.deb")
    except: pass
    try:
        subprocess.check_output("sudo getent passwd xtreamcodes > /dev/null".split())
    except:
        # Create User
...
def install(rType="MAIN"):
    ...
    try: rURL = rDownloadURL[rInstall[rType]]
        ...
        return False
    subprocess.check_call('wget -q -O "/tmp/xtreamcodes.tar.gz" "%s"' % rURL, shell=True)
    if os.path.exists("/tmp/xtreamcodes.tar.gz"):
        printc("Installing Software")
        if os.path.exists("/home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb"):
            subprocess.check_call('chattr -f -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null', shell=True)
        subprocess.check_call('tar -zxvf "/tmp/xtreamcodes.tar.gz" -C "/home/xtreamcodes/" > /dev/null', shell=True)
        try: subprocess.check_call("sudo rm /tmp/xtreamcodes.tar.gz")
        except: pass
        return True
...
def installadminpanel():
    ...
    subprocess.check_call('wget -q -O "/tmp/update.zip" "%s"' % rURL, shell=True)
    ...
    subprocess.check_call('wget -q -O "/tmp/update2.zip" "%s"' % rURL2, shell=True)
...
def mysql(rUsername, rPassword):
    ...
    if rCreate:
        ...
        rFile.close()
        subprocess.check_call("sudo service mysql restart > /dev/null", shell=True)
    ...
    for i in range(5):
        ...
        try:
            if rDrop:
                subprocess.check_call('sudo mysql -u root%s -e "DROP USER IF EXISTS \'%s\'@\'%%\'; DROP USER IF EXISTS \'%s\'@\'localhost\'; DROP USER IF EXISTS \'%s\'@\'127.0.0.1\';" > /dev/null' % (rExtra, rUsername, rUsername, rUsername), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "DROP DATABASE IF EXISTS xtream_iptvpro; CREATE DATABASE IF NOT EXISTS xtream_iptvpro;" > /dev/null' % rExtra, shell=True)
                subprocess.check_call("sudo mysql -u root%s xtream_iptvpro < /home/xtreamcodes/iptv_xtream_codes/database.sql > /dev/null" % rExtra, shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "USE xtream_iptvpro; UPDATE settings SET live_streaming_pass = \'%s\', unique_id = \'%s\', crypt_load_balancing = \'%s\', get_real_ip_client=\'\';" > /dev/null' % (rExtra, generate(20), generate(12), generate(20)), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "USE xtream_iptvpro; REPLACE INTO streaming_servers (id, server_name, domain_name, server_ip, vpn_ip, ssh_password, ssh_port, diff_time_main, http_broadcast_port, total_clients, system_os, network_interface, latency, status, enable_geoip, geoip_countries, last_check_ago, can_delete, server_hardware, total_services, persistent_connections, rtmp_port, geoip_type, isp_names, isp_type, enable_isp, boost_fpm, http_ports_add, network_guaranteed_speed, https_broadcast_port, https_ports_add, whitelist_ips, watchdog_data, timeshift_only) VALUES (1, \'Main Server\', \'\', \'%s\', \'\', NULL, NULL, 0, 8080, 1000, \'%s\', \'eth0\', 0, 1, 0, \'\', 0, 0, \'{}\', 3, 0, 8880, \'low_priority\', \'\', \'low_priority\', 0, 1, \'\', 1000, 8443, \'\', \'[\"127.0.0.1\",\"\"]\', \'{}\', 0);" > /dev/null' % (rExtra, getIP(), getVersion()), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "USE xtream_iptvpro; REPLACE INTO reg_users (id, username, password, email, member_group_id, verified, status) VALUES (1, \'admin\', \'\$6\$rounds=20000\$xtreamcodes\$XThC5OwfuS0YwS4ahiifzF14vkGbGsFF1w7ETL4sRRC5sOrAWCjWvQJDromZUQoQuwbAXAFdX3h3Cp3vqulpS0\', \'admin@website.com\', 1, 1, 1);" > /dev/null'  % rExtra, shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "CREATE USER \'%s\'@\'localhost\' IDENTIFIED BY \'%s\'; GRANT ALL PRIVILEGES ON xtream_iptvpro.* TO \'%s\'@\'localhost\' WITH GRANT OPTION; GRANT SELECT, PROCESS, LOCK TABLES ON *.* TO \'%s\'@\'localhost\';FLUSH PRIVILEGES;" > /dev/null' % (rExtra, rUsername, rPassword, rUsername, rUsername), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "CREATE USER \'%s\'@\'127.0.0.1\' IDENTIFIED BY \'%s\'; GRANT ALL PRIVILEGES ON xtream_iptvpro.* TO \'%s\'@\'127.0.0.1\' WITH GRANT OPTION; GRANT SELECT, PROCESS, LOCK TABLES ON *.* TO \'%s\'@\'127.0.0.1\';FLUSH PRIVILEGES;" > /dev/null' % (rExtra, rUsername, rPassword, rUsername, rUsername), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "GRANT SELECT, INSERT, UPDATE, DELETE ON xtream_iptvpro.* TO \'%s\'@\'%%\' IDENTIFIED BY \'%s\';FLUSH PRIVILEGES;" > /dev/null' % (rExtra, rUsername, rPassword), shell=True)
                subprocess.check_call('sudo mysql -u root%s -e "USE xtream_iptvpro; CREATE TABLE IF NOT EXISTS dashboard_statistics (id int(11) NOT NULL AUTO_INCREMENT, type varchar(16) NOT NULL DEFAULT \'\', time int(16) NOT NULL DEFAULT \'0\', count int(16) NOT NULL DEFAULT \'0\', PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=latin1; INSERT INTO dashboard_statistics (type, time, count) VALUES(\'conns\', UNIX_TIMESTAMP(), 0),(\'users\', UNIX_TIMESTAMP(), 0);\" > /dev/null' % rExtra, shell=True)
                ...
                subprocess.check_call('sudo mysql -u root%s -e "USE xtream_iptvpro; UPDATE settings SET get_real_ip_client=\'\', double_auth=\'1\', hash_lb=\'1\', mag_security=\'1\' where id=\'1\';" > /dev/null'  % rExtra, shell=True)
